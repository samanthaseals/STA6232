...
- In the last lecture, we discussed modeling binary outcomes using logistic regression,
...
- Because `summary()` does not return *p*-values, we will turn to `car::Anova()`.
...
- We will check this assumption with Brant's test ((https://www.jstor.org/stable/2532457)[article here]).
    ...
    - Briefly, this will construct a $\chi^2$ test for every predictor in the model as well as overall (omnibus).
    
    - We look at omnibus first, then individual predictors.
...
- The omnibus test results in $p > \alpha$, thus, we meet the proportional odds assumption.
